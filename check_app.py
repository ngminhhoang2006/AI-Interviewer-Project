...
def find_candidate_folders(name: str) -> list[Path]:
    """Finds folders matching candidate search query across BASE_DIR and uploads."""
    target_flat = flatten_text(name)
    search_roots = [BASE_DIR, UPLOAD_FOLDER]
    matches = []
...
def search_candidate():
    ...
    for f in folders:
        ...
        results.append({
            "folder_name": f.name,
            "folder_path": str(f.relative_to(BASE_DIR)),
...
def get_report():
    ...
    folder_path = (BASE_DIR / relative_folder).resolve()
